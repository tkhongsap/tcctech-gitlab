# PowerShell script to create GitLab issues for Dashboard Service and Weekly Updates
# Run this script to create both issues in the gitlab-activity-analysis project

Write-Host "Creating GitLab issues for Dashboard Service project..." -ForegroundColor Green

# Issue 1: Dashboard Service + Frontend UI
Write-Host "Creating Issue 1: Dashboard Service with REST API and Web Interface..." -ForegroundColor Yellow

python scripts/create_issues.py 2 `
...
Leverage all existing analytics logic from /scripts directory." `
  --labels "enhancement" "api" "dashboard" "frontend" "ui" "high-priority"
...
# Issue 2: Weekly Update System
Write-Host "Creating Issue 2: Weekly Team Update System..." -ForegroundColor Yellow
...
- MS Teams channel updates via webhook
...
Build on existing weekly reporting infrastructure in /scripts directory." `
  --labels "enhancement" "automation" "reports" "weekly-updates" "high-priority"
...
Write-Host "Both issues have been created in the gitlab-activity-analysis project." -ForegroundColor Green 